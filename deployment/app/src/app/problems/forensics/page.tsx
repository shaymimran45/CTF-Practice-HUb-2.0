...
export default function ForensicsProblemsPage() {
  ...
  useEffect(() => {
    async function loadProblems() {
      try {
        const data = await getProblemsByCategory("forensics");
        setProblems(data);
      } catch (error) {
        console.error("Failed to load forensics problems:", error);
      } finally {
        setLoading(false);
      }
    }
  ...
  const categoryName = categoryNames["forensics"] || "forensics";
  ...
  return (
    <div className="min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  {problems.map((problem, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <Link 
                          href={`/problems/forensics/${problem.id}`}
        ...
        <div className="bg-gradient-to-r from-purple-600 to-violet-500 rounded-xl shadow-lg p-8 mt-8 text-white">
          <h2 className="text-2xl font-bold mb-4">Forensics Learning Path</h2>
          <p className="text-xl text-gray-200 mb-6">
            Start with basic file analysis and progress to complex digital investigations.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white bg-opacity-10 rounded-lg p-4">
              <h3 className="text-lg font-semibold mb-2">Beginner</h3>
              <ul className="list-disc pl-5 space-y-1 text-sm">
                <li>File Header Analysis</li>
                <li>Basic Steganography</li>
